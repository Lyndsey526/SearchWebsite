...
public class Search extends HttpServlet
{
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try {
            String filterKey =  key.replace("title:", "").replace("http:", "").replace("NOT", "").replace("AND", "").replace("OR", "").trim();
            if(key.contains("title:"))
                reader.queryTitle("%" + filterKey.replace(" ", "%") + "%", (key.contains("NOT") ? "NOT" : ""));
            else if(key.contains("http:"))
                reader.queryHTTP("%" + filterKey.replace(" ", "%") + "%", (key.contains("NOT") ? "NOT" : ""));
            else
                reader.query("%" + filterKey.replace(" ", "%") + "%", (key.contains("NOT") ? "NOT" : ""));
